...
export const getMyArticles = async (authorId, status) => {
    const res = await api.get(`/articles/my-articles?authorId=${authorId}&status=${status}`)
    return res.data
}
...
export const updateArticle = async (id, articleData, authorId) => {
    const res = await api.put(`/articles/${id}?authorId=${authorId}`, articleData)
    return res.data
}

export const submitForReview = async (id, authorId) => {
    const res = await api.patch(`/articles/${id}/submit-review?authorId=${authorId}`)
    return res.data
}
